<div class="bg-white rounded flex items-center w-full max-w-xl mr-4 shadow-sm border border-gray-200">
    <button class="outline-none focus:outline-none">
    <svg class="w-8 text-gray-600 h-5 cursor-pointer" fill="none" stroke-linecap="round" 
        stroke-linejoin="round" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
    </svg>
    </button>
    <input wire:model="term" type="search" name="" id="" placeholder="Search" class="border-0 w-full pl-3 text-sm text-black outline-none focus:outline-none bg-transparent"/>
</div>
